...
- `episodes/3-episode/episode03_seed.json` — title, fact, runtime, FPS, and manifest path.
- `episodes/3-episode/episode03_shotlist.csv` — 26 contiguous shot blocks totaling 240 seconds.
...
& $Blender --background `
  'Purrcilla_Dixon_Dage_Curious_Together_TEMPLATE.blend' `
  --python 'build_episode03.py' -- --overwrite
```
...
& $Blender --background `
  'Purrcilla_Dixon_Dage_EP03_Purr.blend' `
  --python 'render_episode03_preview.py' -- --only shared_final
```
...
```text
episodes/3-episode/EP03_VERIFICATION.json
```
...
& $Blender --background `
  'Purrcilla_Dixon_Dage_EP03_Purr.blend' `
  --python 'render_episode03_video.py' -- `
