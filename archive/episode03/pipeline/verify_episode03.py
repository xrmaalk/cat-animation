...
from pipeline.asset_audit import image_info
...
images = []
missing_source_art = []
atlas_dimension_mismatches = []
atlas_format_mismatches = []
for name in ("Cila", "Dixon", "Dage"):
    ...
    if source_path.is_file():
        ...
        missing_source_art.append(info["source"])
    atlas_path = ROOT / info["atlas"]
    if atlas_path.is_file():
        image_format, dimensions = image_info(atlas_path)
        if dimensions != tuple(info["atlas_size"]):
            atlas_dimension_mismatches.append(name)
        if image_format != "PNG":
            atlas_format_mismatches.append(name)
    check(
        "source_grid_" + name,
    ...
    image = next(
        (node.image for node in sprite.data.materials[0].node_tree.nodes
...
report = {
    ...
    "warnings": {
        ...
        "missing_original_source_sheets": missing_source_art,
        "atlas_dimensions_mismatch_manifest": atlas_dimension_mismatches,
        "atlas_files_not_png": atlas_format_mismatches,
    },
    "counts": {
