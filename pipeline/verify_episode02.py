...
blender --background Purrcilla_Dixon_Dage_EP02_Cardboard_Box.blend \
  --python pipeline/verify_episode02.py
"""
...
EPISODE_DIR = ROOT / "episodes" / "episode02"
SEED = json.loads((EPISODE_DIR / "episode02_seed.json").read_text(encoding="utf-8"))
...
for name in ("Cila", "Dixon", "Dage"):
    ...
    if source_path.is_file():
        check("source_hash_" + name,
              sha256(source_path.read_bytes()).hexdigest() == info["source_sha256"])
    else:
...
path = EPISODE_DIR / "EP02_VERIFICATION.json"
