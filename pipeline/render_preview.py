"""Render inspection frames or a sequence for an animated WebP preview.
    ...
    blender --background Purrcilla_Dixon_Dage_Animated_Short.blend --python pipeline/render_preview.py
    blender --background Purrcilla_Dixon_Dage_Animated_Short.blend --python pipeline/render_preview.py -- --sequence
    python pipeline/encode_preview.py
...
if "--sequence" in sys.argv:
    out_dir = os.path.join(root, "preview_frames", "animated_short", "sequence")
    ...
    out_dir = os.path.join(root, "preview_frames", "animated_short")
    os.makedirs(out_dir, exist_ok=True)
    scene.render.image_settings.file_format = "PNG"
