"""Rebuild the historical 7x10 character atlases for Episodes 2 and 3.

Run with the system Python: ``python pipeline/prepare_episode02_sprites.py``.
Requires Pillow and NumPy. The original Dage source must be restored first.
It never writes the user-approved current 8x11 Dage sheet or its named copy.
The historical source has seven columns and transparent row gutters. Cila and
Dage have ten rows; Dixon has nine.
"""
...
from pipeline.dage_neck_patch import FRONT_NECK_PATCH_CENTERS, MARK_VERSION
...
def process_character(name, filename):
    ...
    atlas_path = OUTPUT_DIR / ("1.png" if name == "Dage" else
                               f"{name}_EP02_Atlas.png")
...
def main():
    missing = [SOURCE_DIR / filename for filename in SOURCES.values()
               if not (SOURCE_DIR / filename).is_file()]
    if missing:
        raise FileNotFoundError(
            "Historical sprite sources missing; no atlases were changed: "
